...
try:
    ...
    salmon.logger.warning(f"arena_db.json not found, will create when needed.")
...
async def do_query(id_list, user_id, region=1):
    ...
    salmon.logger.debug(f"Arena query {payload=}")
    try:
        ...
        res = resp.json
        salmon.logger.debug(f"len(res)={len(res)}")
    except Exception as e:
        salmon.logger.exception(e)
        return None
    if res["code"]:
        salmon.logger.error(f"Arena query failed.\nResponse={res}\nPayload={payload}")
