...
import salmon
from salmon import Service, priv, util
from salmon.configs import twitter as cfg
from salmon.typing import MessageSegment as ms
try:
    ...
    async with client:
        while True:
            try:
                ...
                salmon.logger.error(f"Error {type(e)} Occurred in twitter stream. Restarting stream in 5s.")
...
async def open_stream(client: PeonyClient):
    ...
    salmon.logger.info(f"订阅推主={router.follows.keys()}, {follow_ids=}")
    stream = client.stream.statuses.filter.post(follow=follow_ids)
    async with stream:
        async for tweet in stream:
            salmon.logger.debug("Got twitter event.")
            if peony.events.tweet(tweet):
                ...
                salmon.logger.info(f"推送推文：\n{msg}")
                for s in entry.services:
                    await s.broadcast(msg, f' @{screen_name} 推文', 0.3)
            else:
                salmon.logger.debug("Ignore non-tweet event.")